...
_SCHEMA_VERSION = "sealed-dataset-handoff-v4"
...
_LAST_OPEN = "2026-06-30T20:00:00Z"
...
class ExcludedProviderRow:
    """One exact closure-to-provider-row identity in handoff order."""

    closure_id: str
    provider_row_sha256: str

    def __post_init__(self) -> None:
        _require_identity(self.closure_id, "excluded row closure ID")
        _require_sha256(self.provider_row_sha256, "excluded provider row SHA-256")
...
class DatasetHandoffManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.dataset_schema_version != "candle-dataset-v4":
            raise DatasetHandoffError("dataset handoff requires candle-dataset-v4")
        object.__setattr__(
            self,
            "closure_manifest_path",
        ...
        if (self.closure_count, self.exclusion_count, self.segment_count) != _EXPECTED_COUNTS:
        ...
        if len(set(self.closure_ids)) != len(self.closure_ids):
            raise DatasetHandoffError("duplicate dataset handoff closure ID")
        for closure_id in self.closure_ids:
            _require_identity(closure_id, "closure ID")
        if len(self.excluded_provider_rows) != self.exclusion_count:
            raise DatasetHandoffError("dataset handoff excluded row count mismatch")
        excluded_closure_ids = tuple(item.closure_id for item in self.excluded_provider_rows)
        excluded_digests = tuple(item.provider_row_sha256 for item in self.excluded_provider_rows)
        if excluded_closure_ids != self.closure_ids:
            raise DatasetHandoffError("dataset handoff excluded row order mismatch")
        if len(set(excluded_closure_ids)) != len(excluded_closure_ids):
            raise DatasetHandoffError("duplicate dataset handoff excluded closure ID")
        if len(set(excluded_digests)) != len(excluded_digests):
            raise DatasetHandoffError("duplicate dataset handoff excluded row digest")
        ...
        if self.candle_count != _EXPECTED_CANDLE_COUNT:
            raise DatasetHandoffError("dataset handoff candle count mismatch")
        if self.first_open_time != _START or self.last_open_time != _LAST_OPEN:
...
def _excluded_rows(mapping: dict[str, object]) -> tuple[ExcludedProviderRow, ...]:
    value = mapping.get("excluded_provider_rows")
    if not isinstance(value, list):
        raise DatasetHandoffError("invalid dataset handoff field: excluded_provider_rows")
    rows: list[ExcludedProviderRow] = []
    for raw_row in cast(list[object], value):
        row_mapping = _mapping(raw_row, "excluded provider row")
        _exact_fields(row_mapping, _EXCLUDED_ROW_FIELDS, "excluded provider row")
        rows.append(
            ExcludedProviderRow(
                closure_id=_str(row_mapping, "closure_id"),
                provider_row_sha256=_str(row_mapping, "provider_row_sha256"),
            )
        )
    return tuple(rows)
...
def verify_dataset_handoff(
    ...
    *,
    ...
    try:
        ...
        if evidence_pairs != manifest.excluded_provider_rows:
            raise DatasetHandoffError("excluded provider row identity mismatch")
        if tuple(item.closure_id for item in evidence_pairs) != closure_ids:
            raise DatasetHandoffError("exclusion closure ID mismatch")
        for closure, exclusion in zip(
            closure_manifest.closures,
            exclusion_manifest.exclusions,
            strict=True,
        ):
            if closure.partial_candle.provider_row_sha256 != exclusion.provider_row_sha256:
                raise DatasetHandoffError("closure and exclusion row identity mismatch")
