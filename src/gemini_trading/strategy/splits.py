"""Sealed and development-only chronological splits for Candidate research."""
...
_V0_2_DEVELOPMENT_START = datetime(2018, 1, 1, tzinfo=UTC)
_V0_2_DEVELOPMENT_END = datetime(2026, 7, 1, tzinfo=UTC)
_V0_2_DEVELOPMENT_FOLD_COUNT = 12
...
@dataclass(frozen=True, slots=True)
class DevelopmentQualificationPlan:
    """Candidate v0.2 development-only folds over the complete fixed historical era."""
    ...
    def __post_init__(self) -> None:
        if self.schema_version != "candidate-development-qualification-plan-v1":
            raise ValueError("unsupported development qualification plan schema")
        if (
            self.dataset_start_time != _V0_2_DEVELOPMENT_START
            or self.dataset_end_exclusive != _V0_2_DEVELOPMENT_END
        ):
            raise SplitBoundaryError("v0.2 development dataset window changed")
        if len(self.folds) != _V0_2_DEVELOPMENT_FOLD_COUNT:
            raise InsufficientHistoryError("v0.2 development plan requires exactly 12 folds")
        if tuple(fold.fold_number for fold in self.folds) != tuple(
            range(1, _V0_2_DEVELOPMENT_FOLD_COUNT + 1)
        ):
            raise SplitBoundaryError("development qualification fold numbering changed")
        if self.boundary_indices != tuple(sorted(set(self.boundary_indices))):
            raise SplitBoundaryError("boundary_indices must be unique and ordered")
        if self.segment_boundary_indices != tuple(sorted(set(self.segment_boundary_indices))):
            raise SplitBoundaryError("segment_boundary_indices must be unique and ordered")
    ...
    def build(
        ...
        policy: CandidatePolicy,
        segment_manifest: CandleSegmentManifest | None = None,
    ) -> "DevelopmentQualificationPlan":
        """Build all locked v0.2 development folds without constructing a final test."""

        if (
            policy.strategy_id != "candidate.multi_model.v0_2"
            or policy.policy_version != "candidate-multi-model-v0.2"
        ):
            raise SplitBoundaryError("development qualification requires Candidate v0.2")
        dataset_start, dataset_end = _validate_candles(candles, segment_manifest)
        if dataset_start != _V0_2_DEVELOPMENT_START or dataset_end != _V0_2_DEVELOPMENT_END:
            raise SplitBoundaryError("v0.2 development dataset window changed")
        eligible = _validate_eligible_indices(eligible_indices, len(candles))
        if not eligible:
            raise InsufficientHistoryError("candidate study has no eligible observations")
        ...
        while True:
            ...
            step += 1
        if len(raw_fold_boundaries) != _V0_2_DEVELOPMENT_FOLD_COUNT:
            raise InsufficientHistoryError("v0.2 development plan requires exactly 12 folds")
        ...
        return cls(
            schema_version="candidate-development-qualification-plan-v1",
            dataset_start_time=dataset_start,
            dataset_end_exclusive=dataset_end,
            folds=tuple(folds),
            boundary_indices=boundaries,
