...
_DOCUMENTS = (
    ...
    / "architecture"
    / "adr"
    / "0003-deterministic-research-engine.md",
    ...
    / "reports"
    / "verification"
    / "deterministic-backtesting-final.md",
)


def test_deterministic_backtesting_documents_preserve_commands_and_trust_boundaries(
) -> None:
    combined = "\n".join(
        path.read_text(encoding="utf-8") for path in _DOCUMENTS
    ).lower()
