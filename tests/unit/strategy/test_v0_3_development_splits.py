...
def _api() -> Any:
    module = importlib.import_module("gemini_trading.strategy.v0_3_splits")
    plan_type = getattr(module, "V03DevelopmentQualificationPlan", None)
    assert plan_type is not None, "v0_3_splits must define V03DevelopmentQualificationPlan"
    return plan_type
...
def test_v0_3_plan_locks_cutoff_and_all_twelve_complete_folds() -> None:
    ...
    plan = _api().build(candles, _eligible(len(candles)), policy)
