...
def test_v0_4_stage1_derives_exact_hourly_scope_from_frozen_source_evidence() -> None:
    ...
    assert hourly.start_time == V04_STAGE1_START
    assert hourly.end_time == V04_STAGE1_END_EXCLUSIVE
    ...
    assert load_v0_4_closure_manifest(hourly_raw) == hourly
    assert serialize_v0_4_closure_manifest(hourly) == hourly_raw
...
def test_v0_4_hourly_derivation_excludes_only_the_actual_partial_hour() -> None:
    hourly, _ = build_v0_4_closure_manifest(_PROJECT_ROOT)
